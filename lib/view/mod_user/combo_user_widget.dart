import 'package:flutter/material.dart';
import 'package:sgii_front/model/cls_000_db_obj.dart';
import 'package:sgii_front/model/cls_user.dart';
import 'package:sgii_front/service/serv_user.dart';
import 'package:sgii_front/util/common/item_list.dart';
import 'package:sgii_front/util/common/result.dart';
import 'package:sgii_front/util/my_widget/combo_widget.dart';

class ComboUserWidget extends StatefulWidget {
  User? Function() getItem;
  void Function(User? item) setItem;
  ComboUserWidget({
    super.key,
    required this.getItem,
    required this.setItem,
  });

  @override
  ComboUserWidgetState createState() => ComboUserWidgetState();
}

class ComboUserWidgetState extends State<ComboUserWidget> {
  final TextEditingController _userController = TextEditingController();
  User userSelect = User.empty();
  UserService servUser = UserService();

  @override
  void initState() {
    super.initState();
    value = widget.getItem();
    if (value != null){
      _userController.text = value!.getValueStr();
    }
  }

  String? validateNotEmpty(String? value) {
    if (value == null || value.isEmpty) {
      return 'Este campo no puede estar vacío';
    }
    return null;
  }
  User? value;
  @override
  Widget build(BuildContext context) {
    return ComboWidget<User>(
      labelText: 'User',
      hintText: 'Seleccione el User',
      controller: _userController,
      validator: validateNotEmpty,
      onSelectedItem: (ItemList<DbObj> item_){
        value = item_.value as User;
        widget.setItem(value);
        _userController.text = value!.getValueStr();
      },
      itemCard: (BuildContext context, ItemList<DbObj> item_) {
        User item = item_.value as User;
        Color backColor = Colors.white60;
        value = widget.getItem();
        if (value != null){
          User aux = value!;
          if (aux.idApi > 0){//Aqui posible bug
            if (aux.idApi == item.idApi){
              backColor = Colors.blueAccent;
            }
          }else{
            if (aux.idApi == item.idApi){
              backColor = Colors.blueAccent;
            }
          }
        }
        return Container(
          padding: EdgeInsets.all(8),
          color: backColor,
          child: Text(item.getValueStr()),
        );
      },
      fetchData: (DateTime offset, int take, bool forceRefresh, Future<void> Function() reset) async {
        ResultOf<List<User>> result = await servUser.fetchDataFind(offset, take, null);
        return result;
      },
    );
  }
}
