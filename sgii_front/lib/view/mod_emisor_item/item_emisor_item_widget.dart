...
class ItemEmisorItemWidget extends StatefulWidget {
  final ItemList<DbObj> item;
  final GlobalKey<SearchListWidgetState<EmisorItem>>? searchListWidgetKey;
  final GlobalKey<ListWidgetState<EmisorItem>>? listWidgetKey;
  final Future<void> Function(BuildContext context, ItemList<DbObj> item, void Function() fSetState) selectedItem;

  ItemEmisorItemWidget({
    super.key,
    required this.item,
  ...
  ItemEmisorItemWidgetState createState() => ItemEmisorItemWidgetState();
}
class ItemEmisorItemWidgetState extends State<ItemEmisorItemWidget>{
  EmisorItemService serv = EmisorItemService();

  @override
  Widget build(BuildContext context) {
    EmisorItem value = widget.item.value as EmisorItem;
    widget.item.reset = (){
      if (mounted) {
        setState(() {});
      }
                  ...
                  onPressed: () {
                    showDialog(
                      ...
                      builder: (BuildContext context) {
                        return AlertDialog(
                          title: Text('Detalles del EmisorItem'),
                          content: SingleChildScrollView(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text('EmisorEstablecimiento: ${value.emisorEstablecimiento.getValueStr()}'),
                                Text('PuntoEmision: ${value.puntoEmision}'),
                              ],
                            ),
                          ),
                          actions: <Widget>[
